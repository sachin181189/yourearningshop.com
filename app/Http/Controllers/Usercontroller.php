...
class Usercontroller extends Controller
{
    ...
    public function confirm_order(Request $request)
    {
        ...
        $cartdata = Cart::select('cart.id','cart.offer_price','cart.price','product.product_type','product.vendor_id','product.product_name','product.product_image','cart.qty','cart.product_id','cart.variant_value1','cart.variant_value2','vendor.vendor_name','vendor.company_name','vendor.email','vendor.mobile','vendor.address','vendor.alternate_mobile','vendor.service_description','vendor.gst_no','vendor.city','vendor.state','vendor.zip','product.return_exchange_policy_type','product.return_exchange_days','category.category','subcategory.subcategory','brand.brand_name')
        ->join('product','product.id', '=', 'cart.product_id')
        ...
        $shipping_address_data = DB::table('user_address')->select('*')
        ->where('id',$shipping_address)
        ->first();

        $zip = $shipping_address_data->zip;
        foreach($cartdata as $c)
        {
            ...
            $productdt = array(
                ...
                'return_exchange_policy_type'=>$c->return_exchange_policy_type,
                'return_exchange_days'=>$c->return_exchange_days,
                ...
                'email'=>$c->email,
                'mobile'=>$c->mobile,
                'vendor_alternate_mobile'=>$c->alternate_mobile,
                'address'=>$c->address,
                'city'=>$c->city,
                'state'=>$c->state,
                'zip'=>$c->zip,
        ...
        if($is_same_as_shipping_address == 0){
            $billing_address_data = DB::table('billing_address')->select('*')
            ->where('id',$billing_address)
            ->first();
        }else{
        ...
        $orderarr = array(
            ...
            'shipping_user_name'=>$shipping_address_data->fname.' '.$shipping_address_data->lname,
            'shipping_email'=>$shipping_address_data->email,
            ...
            'shipping_flat'=>$shipping_address_data->flat,
            'shipping_area'=>$shipping_address_data->area,
            'shipping_landmark'=>$shipping_address_data->landmark,
            'shipping_address_type'=>($shipping_address_data->address_type == 1) ? "Home" : "Office",
            'billing_name'=>($is_same_as_shipping_address == 0) ? $billing_address_data->user_name : $shipping_address_data->fname.' '.$shipping_address_data->lname,
            ...
            'billing_email'=>$billing_address_data->email,
    ...
    public function getConnection(Request $request)
    {
        ...
        //                 ->get();
        $connection = User::selectRaw('users.*,(Select count(id) from users u where u.parent_referral_code=users.referral_code) as total_connection, 0 as total_earning')
        ->where('parent_referral_code', $referral_code)
        ->get();
    ...
    public function myAccount()
    {
        ...
        if($user_id)
        {
            ...
            foreach($orderdata as $od)
            {
                ...
                $order_product_detail = OrderProduct::select('*')
                ->where('order_id',$order_id)
                ->get();
                $od['order_product'] = $order_product_detail;
                $orders[]=$od;
            }
            }
            else
